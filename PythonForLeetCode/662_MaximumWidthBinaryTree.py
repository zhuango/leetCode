    ...
    def widthOfBinaryTree(self, root):
        ...
        while index < len(queue):
            ...
            if curTuple[1] > level:
                level = curTuple[1]
            if curTuple[0].left:
                queue.append((curTuple[0].left, curTuple[1] + 1, curTuple[2] * 2))
            if curTuple[0].right:
                queue.append((curTuple[0].right, curTuple[1] + 1, curTuple[2] * 2 + 1))
            
            if curTuple[1] != tempLevel:
                dis = levelEnd - levelStart + 1
                if dis > result:
                    result = dis
                levelStart = levelEnd = curTuple[2]
                tempLevel += 1
            else:
                levelEnd = curTuple[2]
            index += 1
        
        return max([result, levelEnd - levelStart + 1])
